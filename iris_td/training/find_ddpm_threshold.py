...
def main() -> None:
    ...
    # SCORE INVERSION FIX: For AnoDDPM with partial noising, attacks reconstruct
    # more faithfully than bonafide (t*=500 preserves attack texture), so
    # attack MSE < bonafide MSE — the opposite of what we expect.
    # Negate reconstruction scores so that attacks score HIGHER than bonafide,
    # making the standard "score > threshold → attack" convention correct.
    for neg_col in ["mse_score", "lpips_score", "recon_score"]:
        if neg_col in df.columns:
            df[neg_col] = -df[neg_col]
            logger.info(f"Negated {neg_col} scores (inversion fix)")
    ...
    # Visualizations
    vis_dir = IRIS_TD_ROOT / "results" / "visualizations"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scores_csv",
        type=str,
        default="iris_td/pad_scores/ddpm_val_gaussian_tstar500_steps100.csv",
        help="Path to validation PAD scores CSV.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="iris_td/results/ddpm_threshold.json",
